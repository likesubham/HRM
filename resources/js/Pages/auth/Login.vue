    ...
    <div class="bg-surface-0 dark:bg-surface-900 p-8 md:p-12 shadow-sm rounded-2xl w-full max-w-xl mx-auto flex flex-col gap-8">
      ...
      <form @submit.prevent="handleLogin" class="flex flex-col gap-6 w-full">
        <div class="flex flex-col gap-2 w-full">
          ...
          <InputText id="email1" v-model="email" type="email" placeholder="Email address" class="w-full px-3 py-2 shadow-sm rounded-lg" required />
        </div>
        <div class="flex flex-col gap-2 w-full">
          <label for="password1" class="text-surface-900 dark:text-surface-0 font-medium leading-normal">Password</label>
          <InputText id="password1" v-model="password" type="password" placeholder="Password" class="w-full px-3 py-2 shadow-sm rounded-lg" required />
        </div>
        ...
        <Button label="Sign In" icon="pi pi-user" class="w-full py-2 rounded-lg flex justify-center items-center gap-2" type="submit">
          <template #icon>
...
<style scoped>
/* You can add custom styles here or rely on Tailwind/PrimeVue classes */
</style> 